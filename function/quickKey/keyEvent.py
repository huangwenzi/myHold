...
class KeysEvent():
    ...
    def keyDown(self, data, key):
        ...
        if ret == enums.keysRet.isFun:
            return ret
        elif ret == enums.keysRet.isExit:
            return ret
    ...
    def isFun(self, data, key):
        ...
        if hasattr(key, "char"):
            ...
            if char == ".":
                ...
                if data.openFlag:
                    log_tool.log("quickKey open", enums.log_type.dos)
                else:
                    log_tool.log("quickKey close", enums.log_type.dos)
            # 保存快捷点
                ...
                log_tool.log(data.position, enums.log_type.dos)
            # 截图
            elif char == "+":
                # 检查目标文件夹是否存在
            ...
            # 退出 因为按键的事件是另开线程处理的，这里的退出只是退出处理按键事件的线程，不能退出整个界面
                ...
                log_tool.log("exit", enums.log_type.dos)
                return enums.keysRet.isExit
            # 不存在的功能键就返回false
            else :
                return enums.keysRet.noFun

        return enums.keysRet.isFun
